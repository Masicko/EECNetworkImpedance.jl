begin
  ...
  function find_x_from_y(val, x_data, y_data)
    get_t(R, R_min, R_max) = (R - R_min)/(R_max - R_min)

    found = false
    if val > maximum(y_data)
      return x_data[1]
    elseif val < minimum(y_data)
      return x_data[end]
    end
    for i in 1:length(y_data)-1
        if (y_data[i] <= val && val <= y_data[i+1]) ||
           (y_data[i] >= val && val >= y_data[i+1])
            found = true
            tau = get_t(val, y_data[i], y_data[i+1])
            return x_data[i] + tau*(x_data[i+1] - x_data[i])
        end
    end
    if found == false
  ...
  function get_fit_to_id(id, T_list, prestring, add_prms=[])
    ...
    for T in T_list       
      ...
      x_data = gdf[1][:, :"p.R_LSM"]
      y_data = gdf[1][:, :R_mean]
      T_idx = findall(x -> x == T, T_all)[1]
      res_R_LSM = find_x_from_y(1/data_conductivity_all[id, T_idx], x_data, y_data)
